...
parser.add_argument('--rand-prune', action="store_true")
parser.add_argument('--reinit', action="store_true")
parser.add_argument('--randseed', type=int, help='random seed', default=None)
args = parser.parse_args()

# assert args.resume != "" and isfile(args.resume), "Pretrained model must be specified!"
...
def get_factors(model):
    ...
    for idx, (name, m) in enumerate(named_modules.items()):
        if isinstance(m, nn.BatchNorm2d):
            ...
            if args.rand_prune:
                factors[name] = torch.rand(factors[name].numel())
...
def main():
    ...
    # model and optimizer
    model_str = "vgg_cifar.vgg16_bn(num_classes=100).cuda()"
    ...
    logger.info("evaluating after resuming...")
    ...
    logger.info("Parameter details after real pruning:")
    for name, p in model.named_parameters():
        logger.info("%s shape %s"%(name, p.shape))
    ...
    if args.reinit:
        ...
        logger.info("evaluating after reinitiation...")
        validate(val_loader, model, args.epochs)
